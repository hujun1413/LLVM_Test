				...
				if(OpCode == Instruction::ICmp)    //是if判断
				{
					if (MDNode *N = MyIn->getMetadata("dbg")) 
					{
						...
						if(Line == line)    //在某行
						{
							...
							//Compare with constant int or null pointer	 
							int const_para = 0;	 	 
							if (mycmp->getOperand(1)->getValueID() == Value::ConstantIntVal)	 	 	 
							{	
								const_para = 0;
							}
							else if(mycmp->getOperand(0)->getValueID() == Value::ConstantIntVal)
							{
								const_para = 1;
							}
							else
							...
							if (Instruction* inst = dyn_cast<Instruction>(mycmp->getOperand(const_para))) 
							{
								...
								if(OpCode == Instruction::And)
								{
									...
									{
										outs() << "find LoadInst:" << *valInst<< "    " << *valInst->getType() << "\n";
